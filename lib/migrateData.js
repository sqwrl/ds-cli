                            ...
                            for (var r = 0; r < recs.length; r++) {
                                ...
                                for (prop in document) {
                                    if (document.hasOwnProperty(prop) && document[prop].hasOwnProperty('type')) {
                                        if (document[prop].type === 'nested') {
                                        ...
                                        } else if (document[prop].hasOwnProperty('function')) {
                                            if (document[prop].function.type === 'concatStrings') {
                                                var val = '';
                                                for (var s = 0; s < document[prop].function.fields.length; s++) {
                                                    var string = document[prop].function.fields[s];
                                                    if (string.substring(0, 1) === '|') {
                                                        val += string.replace('|', '');
                                                    } else {
                                                        val += recs[r][document[prop].function.fields[s]];
                                                    }
                                                }
                                            ...
                                            if (document[prop].function.type === 'equation') {
                                                var numval = 0, numnext = 0, num = 0, op = '+', operator = false;
                                                for (var n = 0; n < document[prop].function.fields.length; n++) {
                                                    var number = document[prop].function.fields[n];
                                                    if (number.substring(0, 1) === '|') {
                                                        numnext = number.replace('|', '');
                                                        if (!isNaN(numnext)) {
                                                            num = Number(numnext);
                                                            switch (op) {
                                                                case '+':
                                                        ...
                                                        if (number.substring(0, 1) === '#') {
                                                            var numvalorig = numval;
                                                            var newnum = number.replace('#','');
                                                            newnum = newnum.replace('value', numval);
                                                            try {
                                                                numval = eval(newnum);
                                                            } catch(e) {
                                                                logger.error(e, 'error evaluating equation function');
                                                                numval = numvalorig;
                                                            }

                                                        } else {
                                                            var fieldValue = recs[r][document[prop].function.fields[n]];
                                                            if (!isNaN(fieldValue)) {
                                                                fieldValue = Number(fieldValue);
                                                                if (operator) {
                                                                    switch (op) {
                                                                        case '+':
                                                                            numval = numval + fieldValue;
                                                                            break;
                                                                        case '-':
                                                                            numval = numval - fieldValue;
                                                                            break;
                                                                    ...
                                                                    numval = Number(fieldValue);
                                                                }
                                                            } else {
                                                                logger.error(fieldValue, 'equation: not a number');
                                                            }
                                                        }
                                                    }
                                                }
                                        ...
                                        else if (document[prop].hasOwnProperty('code')) {

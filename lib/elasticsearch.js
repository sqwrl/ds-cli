...
module.exports.createIndexMapping = function(ss, cb) {
    ...
    async.eachSeries(
        ...
        function(index, callback) {
            ...
            function fieldIsString(fi, obj, dflt) {
                obj.analyzer = fi.analyzer || dflt.analyzer;
                if (fi.facet) {
                    obj.fields = {
                        raw: {
                            type: 'string',
            ...
            function fieldIsDate(fi, obj, dflt) {
                obj.format = fi.format || dflt.format;
            }

            // extract the mappings properties
            for (var field in index.doc) {
                if (index.doc.hasOwnProperty(field)) {

                    // generate the field mapping
                    var fieldInfo;
                    if (index.doc.hasOwnProperty(field)) {
                        fieldInfo = index.doc[field];
                    }

                    if (fieldInfo.type !== 'nested') {
                        var dflt, obj;
                        if (fieldInfo.hasOwnProperty('type')) {
                            dflt = _.get(defaults, fieldInfo.type);
                            obj = {
                                type: fieldInfo.type,
                            ...
                            switch (fieldInfo.type) {
                                case 'string':
                                    fieldIsString(fieldInfo, obj, dflt);
                                    break;
                                case 'date':
                                    fieldIsDate(fieldInfo, obj, dflt);
                                    break;
                            }

                            _.set(properties, field, obj);
                        ...
                        _.set(meta, field + '.type', 'nested');
                        _.set(meta, field + '.text', fieldInfo.text);

                        for (var subField in fieldInfo.doc) {
                            var dflt2, obj2, subFieldInfo;
                            if (fieldInfo.doc.hasOwnProperty(subField)) {
                                subFieldInfo = fieldInfo.doc[subField];
                                dflt2 = _.get(defaults, subFieldInfo.type);
                                obj2 = {
                                    type: subFieldInfo.type,
                                    store: subFieldInfo.store || dflt2.store,
                                    index: subFieldInfo.index || dflt2.index
                                };

                                switch (subFieldInfo.type) {
                                    case 'string':
                                        fieldIsString(subFieldInfo, obj2, dflt2);
                                        break;
                                    case 'date':
                                        fieldIsDate(subFieldInfo, obj2, dflt2);
                                        break;
                                }

                                _.set(properties, field + '.properties.' + subField, obj2);

                                // generate the _meta information
                                _.set(meta, field + '.fields.' + subField, {
                                    text: subFieldInfo.text,
                                    type: subFieldInfo.type,
                                    facet: subFieldInfo.facet || false
                                });
                            }
                        }
                    }

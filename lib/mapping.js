 ...
 * @param {Function} cb - callback function when operation complete
 */
module.exports.createIndexMapping = function(type, version, ss, cb) {
    ...
    for (var ft = 0; ft < mapPathIndexTypes.length; ft++) {
        ...
        logger.debug('loop through types: ' + indexType);
                ...
                module.exports.loadFile(file, function (err, mapped, file) {
                    if (!err) {
                        if (file.match('generic')) {
                            ...
                            if (mapped !== undefined) {
                                if (mapped.hasOwnProperty('doc') || mapped.hasOwnProperty('subDocs')) {
                                    var toMap = {
                                        type: indexType,
                                        doc: mapped
                                    };
